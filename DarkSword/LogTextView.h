...
void log_init(void);
void log_write(const char * _Nonnull msg);
void log_write_raw_no_timestamp(const char * _Nonnull msg);
void log_set_verbose(BOOL enabled);
BOOL log_verbose_enabled(void);
void log_user(const char * _Nonnull fmt, ...) __attribute__((format(printf, 1, 2)));
...
NSString * _Nonnull log_inapp_buffer_snapshot(void);
