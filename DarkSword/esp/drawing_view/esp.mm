...
static inline void ESPAddHealthBar(CGMutablePathRef bgPath, CGMutablePathRef fillPath,
    ...
    // 👉 THANH MÁU MỎNG 0.5f
    const CGFloat barW = 0.5f;
    const CGFloat gap   = 1.5f;
    const CGFloat bdr   = 0.3f;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    // DarkSword remake: CADisplayLink tạm dừng khi app vào nền (user đang
    // ở màn hình game) — ESP host sẽ driving updateFrame bằng dispatch
    // timer trên main queue (app giữ nền bằng audio keep-alive).
...
// 👉 FOV - MÀU XANH NƯỚC BIỂN (CYAN)
UIColor *cyan = [UIColor colorWithRed:0.0f green:1.0f blue:1.0f alpha:0.6f];
self.fovLayer = [self makeShapeLayer:cyan
                                fill:nil
                           lineWidth:0.7f
                                zPos:0];
    ...
    self.hpBackgroundLayer = [self makeShapeLayer:[UIColor blackColor] fill:nil lineWidth:0.3f zPos:4];
...
// DarkSword: public tick — ESP host gọi ở 60Hz từ dispatch timer.
- (void)tickFrame {
    [self updateFrame];
}
...
- (void)updateFrame {
    ...
    @autoreleasepool {
        ...
        esp_set_enemy_count(stats.playerCount + stats.botCount);
...
// 👉 SỐ ĐỊCH - HIỆN "05EX FF" KHI 0 ĐỊCH, MÀU XANH NƯỚC BIỂN, KHÔNG NỀN
if (isCount) {
    int totalEnemies = stats.playerCount + stats.botCount;
    
    if (totalEnemies > 0) {
        // Có địch -> hiện số
        self.statusLayer.string = [NSString stringWithFormat:@"%d", totalEnemies];
        self.statusLayer.fontSize = 20.0f;
        self.statusLayer.font = (__bridge CFTypeRef)[UIFont boldSystemFontOfSize:20.0f];
        self.statusLayer.frame = CGRectMake(vw / 2.0f - 20.0f, 30.0f, 40.0f, 30.0f);
    } else {
        // Không có địch -> hiện "05EX FF"
        self.statusLayer.string = @"05EX FF";
        self.statusLayer.fontSize = 20.0f;
        self.statusLayer.font = (__bridge CFTypeRef)[UIFont boldSystemFontOfSize:20.0f];
        self.statusLayer.frame = CGRectMake(vw / 2.0f - 40.0f, 30.0f, 80.0f, 30.0f);
    }
    
    self.statusLayer.foregroundColor = [UIColor colorWithRed:0.0f green:1.0f blue:1.0f alpha:1.0f].CGColor;
    self.statusLayer.backgroundColor = [UIColor clearColor].CGColor;
    self.statusLayer.cornerRadius = 0.0f;
