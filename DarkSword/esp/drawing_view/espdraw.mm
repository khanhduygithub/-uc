...
void RenderESPForPawn(
    ...
    if (isHealth) {
        ...
        if (MaxHP > 0) {
            ...
            // 🟢 SỬA: barWidth = 1.0f, xịch sát gần box
            const CGFloat barWidth = 1.0f;        // Giảm từ 3.0f xuống 1.0f
            const CGFloat barGap   = 1.5f;        // Khoảng cách nhỏ xíu
            const CGFloat borderThick = 0.5f;     // Viền đen mỏng
