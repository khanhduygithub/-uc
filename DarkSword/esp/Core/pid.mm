...
static bool cachedBaseReset = false;   // DarkSword auto-heal flag
...
uintptr_t GetGameModule_Base(char *GameProcessName) {
    // 🟢 Cache lại module base để không gọi lại mỗi lần (giảm lag)
    static uintptr_t cachedBase = 0;
    if (cachedBaseReset) { cachedBase = 0; cachedBaseReset = false; }
    if (cachedBase != 0) return cachedBase;

    pid_t pid = GetGameProcesspid(GameProcessName);
    if (pid == -1) return 0;

    // 🟢 Sử dụng task_for_pid để lấy quyền truy cập process
    get_task = get_task_for_PID(pid);
    if (get_task == MACH_PORT_NULL) {
        return 0;
    }

    // 🟢 Tìm chính xác module bằng dyld info
    const char *imageTry[] = {
        "UnityFramework",
...
void esp_memory_reset_cache(void)
{
    // cachedBase là static trong GetGameModule_Base — reset qua flag nội bộ
    cachedBaseReset = true;
...
{
    // DarkSword remake: lấy task port của game qua kernel R/W bridge
    // (port transplant) thay cho entitlement task_for_pid-allow của
    // TrollStore. task_for_pid thật giữ lại làm fallback cho bản build
    // TrollStore — sẽ thất bại im lặng trên bản DarkSword thường.
    mach_port_t task = esp_krw_task_port();
...
pid_t get_pid_by_name(const char *keyword)
{
    ...
    size_t pids_size = (size_t)count * sizeof(pid_t);
    pid_t *pids = (pid_t *)malloc(pids_size);
    if (!pids) return -1;

    proc_listallpids(pids, (int)pids_size);
