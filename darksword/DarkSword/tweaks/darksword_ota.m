...
static bool ota_disable_original_remote_call(void)
{
    ...
    if (!krw_launchd_remote_call_allowed()) {
        printf("[ota] refusing launchd hijack — pid 1 anchor disabled by safety policy. Use kernel-local path instead.\n");
        log_user("[OTA] Refusing to hijack launchd (pid 1) — safety policy active. Run the chain via the local root-rw path instead.\n");
        return false;
    }
