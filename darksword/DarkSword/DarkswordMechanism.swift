...
enum DarkswordMechanism {
    ...
    static func applySessionTweaks(_ config: Config,
        ...
        queue.async {
            ...
            let ownedSession = ensureSession(&report)
            guard ownedSession else {
                completion(report)
                return
            }
            ...
            // Keep the session alive if the RSSI loop is running.
            if rssiTimer == nil {
                closeSession()
            } else {
                log("mechanism: giữ session mở — vòng RSSI đang chạy")
            }
            completion(report)
        }
    }
    ...
    static func startRSSILoop(provider: @escaping () -> (Bool, Bool),
        ...
        queue.async {
            ...
            var report = Report()
            guard ensureSession(&report) else {
            ...
            guard rssidisplay_apply_in_session(wifi, cell) else {
                log("mechanism: RSSI tick đầu thất bại — đóng session")
                closeSession()
            ...
            timer.setEventHandler { [weak box = ProviderBox(provider)] in
                guard let provider = box?.provider else { return }
                let (w, c) = provider()
    ...
    // MARK: - Session helpers (queue-only)

    /// Opens the SpringBoard session unless already open (RSSI loop).
    private static func ensureSession(_ report: inout Report) -> Bool {
        if sessionOpen {
            report.sessionOpened = true
            log("mechanism: dùng lại session SpringBoard đang mở (pid=\(remote_call_current_pid()))")
            return true
        }
